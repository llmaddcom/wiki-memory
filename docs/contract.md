...
`{"query": "...", "method": "fuzzy|bm25|llm", "max_pages": 3, "detail": "full|hook"}`
...
- `detail` 取舍：`full`（默认，页面全文，不传时行为与旧版完全一致）→
  ...
  （title/summary/body 全字段计分），hook 只是响应裁剪。
...
score / hook_fallback`，**不含 body/summary**。存量页 hook 尚未回填（为空）时
降级用 summary 前 20 字并标 `hook_fallback: true`；固化触及页面时自然回填。
...
**单页读取（联想 / 溯源）**：`GET /spaces/{uid}/pages/{slug}` 返回页面全字段
（含 `hook` / `happened_on` / `summary` / `body`），并附
`evidence_dates: ["2026-07-01", …]`——该页全部修订关联 source 的 `occurred_at`
日期（YYYY-MM-DD 去重升序）。模型拿到日期即可去翻上游对应日记做更深下钻，
无需再发一次 evidence 请求。JSON 形态，**不是**可直接注入的 `context_block`。

### 2.1 渐进披露（小上下文路径）

推荐两步，避免每轮灌全文：

1. **轻量召回**：`POST /spaces/{uid}/recall`，
   `{"query": "...", "method": "bm25", "detail": "hook"}` → 只注入钩子行
   `context_block`（体积小）。
2. **按需展开**：模型根据 `[[slug]]` 判定相关后，
   `POST /spaces/{uid}/pages/expand`，`{"slugs": ["slug-a", "slug-b"]}`
   （1～10 个）→ 返回与 `detail=full` **同款**的 `context_block`，再注入当前回合。

`expand` 响应：`hits`（按请求顺序的 active 页）+ `missing`（不存在或已归档的
slug，不整单 404）+ `context_block`（无命中为空串，勿注入空块）。
钩子块与展开块分开注入即可；历史回合不要重复堆叠同一批记忆。
