...
| `diary` | `[YYYY-MM-DD] 日记正文`（上游日记板块的产物；当日汇总或逐回合实时转录条目皆可） | 0.2~0.5 |
...
`{"query": "...", "method": "fuzzy|bm25|llm|embedding", "max_pages": 3, "detail": "full|hook"}`

- `query` 用**当前用户输入**（或加上任务概述），不要传整个对话历史。
- method 取舍：`bm25`（默认，毫秒级，词面匹配 + 关键字倒排加分）→
  `fuzzy`（更宽松的词频）→ `llm`（语义最准，一次 LLM 往返延迟，高价值场景用）→
  `embedding`（语义向量，需服务端配置 `WIKIMEM_EMBEDDER_API_BASE`，未配置返回 422）。
- `detail` 取舍：`full`（默认，页面全文，不传时行为与旧版完全一致）→
  `hook`（轻量钩子行，渐进披露场景的逐回合注入）。排序算法与 detail 无关
  （title/hook/summary/body 全字段计分），hook 只是响应裁剪。
- bm25 的 `score` 是 sigmoid 归一合成分（0~1）；`hits[]` 附 `score_details`
  逐信号拆解（`bm25_raw`/`bm25_norm`/`keyword_boost`/`max_possible`/`final`），
  供上游调参与"为什么想起这条"的溯源展示。
- **provisional 命中**：salience ≥ 0.8（服务端可配）的待固化材料会以临时文档
  参与 bm25/fuzzy 现算，命中标 `provisional: true`、slug 形如 `pending-{id}`
  （无展开页可点名，钩子行渲染为「…（待固化）」）；固化后自然退出。
  这是"用户刚纠正、还没固化"冷启动真空的兜底。
- 召回命中即记页面 usage（`hit_count`/`last_hit_at`），联想展开另有
  `?track=associate` 通道（见下），目前只积累数据不参与排序。

`detail="full"`（默认）的 `context_block` 是**可直接注入对话**的标准文本块：
...
`detail="hook"` 时每页只渲染一行钩子（≤20 字关键点 + 时间点），`[[slug]]` 供
模型判定相关后点名展开（读单页接口，见下）：

```xml
<recalled_memory>
- [[report-format-mistake]] 日报别用周报格式（2026-07-03）
- [[lao-zhang]] 称呼偏好老张，勿称张先生
</recalled_memory>
```

hook 模式的 `hits[]` 也相应裁剪为 `slug / title / type / hook / happened_on /
score / score_details / provisional / hook_fallback`，**不含 body/summary**。
存量页 hook 尚未回填（为空）时降级用 summary 前 20 字并标
`hook_fallback: true`；固化触及页面时自然回填。
...
**单页展开（联想的数据源）**：`GET /spaces/{uid}/pages/{slug}` 返回页面全字段
（含 `hook` / `happened_on` / `summary` / `body` / `hit_count` / `last_hit_at`），
并附 `evidence_dates: ["2026-07-01", …]`——该页全部修订关联 source 的 `occurred_at`
日期（YYYY-MM-DD 去重升序）。模型拿到日期即可去翻上游对应日记做更深下钻，
无需再发一次 evidence 请求。

**usage 记账通道**：模型经联想工具主动展开页面时，上游应带
`GET /spaces/{uid}/pages/{slug}?track=associate`——这是"记忆真被用上"的最强
信号，命中记入 `hit_count`/`last_hit_at`。预览/审计读取**不带** track 参数、
不计数。usage 是第一方观测数据：REDACT 重写、回滚都保留，绝不清零。

## 3. 固化时机（上游自定）

`POST /spaces/{uid}/consolidate`。服务不内置调度，推荐挂在：
日记生成后（按天）、会话结束时、或 cron。高显著性事件（correction）
之后应立即触发一次小固化（`{"trigger": "correction", "max_sources": 5}`），
实现"刚说的话不转头就忘"；固化前的窗口期由高 salience pending 临时召回兜底。
